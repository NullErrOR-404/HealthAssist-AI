...
export const DrugInfo: React.FC = () => {
  ...
  const handleSearch = async (e?: React.FormEvent, searchOverride?: string) => {
    ...
    setContent(null);
    try {
      const res = await fetch('http://127.0.0.1:8000/drug-info', {
        method: 'POST',
          ...
          <div className="absolute top-[calc(100%-0.5rem)] left-4 right-4 bg-card border border-border rounded-xl shadow-lg z-50 overflow-hidden">
            {suggestions.map((drug) => (
              <button
                key={drug}
                type="button"
                onClick={() => handleSearch(undefined, drug)}
      ...
      <div className="flex-1 overflow-y-auto p-4 pt-2">
        {loading && (
          <div className="flex flex-col items-center justify-center h-48 text-muted-foreground gap-3">
            ...
            <p className="text-sm font-medium animate-pulse">Compiling pharmacology data...</p>
          </div>
        )}
        {error && (
        ...
        {!loading && !error && !content && (
          <div className="flex flex-col items-center justify-center h-48 text-muted-foreground gap-2 opacity-50">
            <Pill size={32} /><p className="text-sm text-center">Search for a medication to generate<br/>a structured clinical profile.</p>
          </div>
        )}
        {content && !loading && (
            ...
            {content.split(/(?=## )/g).map((chunk, idx) => {
              ...
              return (
                <div key={idx} className={isHero ? "mb-2" : "bg-card border border-border rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden"}>
                  <div className="prose prose-sm max-w-none text-[14.5px] leading-relaxed text-foreground prose-headings:text-foreground prose-h1:text-2xl prose-h1:font-extrabold prose-h1:tracking-tight prose-h1:mb-2 prose-h2:text-sm prose-h2:uppercase prose-h2:tracking-widest prose-h2:text-indigo-500 prose-h2:mt-0 prose-h2:mb-4 prose-h2:font-bold prose-li:marker:text-indigo-500 prose-strong:text-foreground prose-p:leading-relaxed prose-li:leading-relaxed prose-table:border-collapse prose-table:w-full prose-th:bg-muted prose-th:p-2 prose-th:border prose-th:border-border prose-td:p-2 prose-td:border prose-td:border-border">
                    <ReactMarkdown
                      remarkPlugins={[remarkGfm]}
                      components={{
                        img: ({node, ...props}) => (
                          <div className="w-full h-56 md:h-64 overflow-hidden rounded-2xl mb-6 shadow-md border border-border relative group cursor-pointer bg-white">
                            <img {...props} className="w-full h-full object-contain p-4 group-hover:scale-105 transition-transform duration-700 ease-out" />
